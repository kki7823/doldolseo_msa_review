...
public class UploadReviewFileUtil extends UploadFileUtil {
    ...
    public String saveReviewImg(String uuid, MultipartFile reviewImgFile) {
        if (reviewImgFile != null) {
            String originalImgFileName = reviewImgFile.getOriginalFilename();
            makeDirIfNoExist(REVIEW_IMG_PATH + uuid);
            Path savePath = Paths.get(REVIEW_IMG_PATH + uuid + "/" + originalImgFileName);
    ...
    public void moveImgsReviewNoDirToTemp(Long reviewNo) {
